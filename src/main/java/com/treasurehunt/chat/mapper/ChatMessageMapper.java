...
public interface ChatMessageMapper extends BaseMapper<ChatMessageDO> {
    ...
    @Insert("INSERT INTO chat_message(\n" +
            "  conversation_id, server_msg_id, client_msg_id, sender_id, from_user_id, from_user_no, sender_no, msg_type, content, payload_json, hash_code, status, push_attempts, shop_id, business_line, created_at, delivered_at\n" +
            ") VALUES (\n" +
            "  #{conversationId}, #{serverMsgId}, #{clientMsgId}, #{senderId}, #{fromUserId}, #{fromUserNo}, #{senderNo}, #{msgType}, #{content}, #{payloadJson}, #{hashCode}, #{status}, #{pushAttempts}, #{shopId}, #{businessLine}, #{createdAt}, #{deliveredAt}\n" +
            ")\n" +
